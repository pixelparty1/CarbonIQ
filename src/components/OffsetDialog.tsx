...
import { Leaf, Sun, Wind, Download } from "lucide-react";
...
// Define offset options with their formulas
...
export function OffsetDialog({ userCredits, onCreditsUpdate, onTransactionComplete }: OffsetDialogProps) {
...
Note: Even small transactions require some ETH for gas fees. Consider adding at least 0.01 ETH to your wallet.`;
  ...
  return (
    <Dialog open={open} onOpenChange={(newOpen) => {
      ...
      <DialogContent className="sm:max-w-[600px]" style={{ background: '#1C1C1C', border: '1px solid #2A2A2A', zIndex: 100 }}>
        <DialogHeader>
          <DialogTitle style={{ color: '#E0E0E0' }}>Offset Your Carbon Footprint</DialogTitle>
          ...
          {OFFSET_OPTIONS.map((option) => (
            ...
            >
              <div className="flex flex-col items-center text-center gap-2">
                ...
                >
                  <option.icon style={{ color: option.color }} className="h-6 w-6" />
                </div>
                <h3 className="font-medium" style={{ color: '#E0E0E0' }}>{option.name}</h3>
        ...
        {selectedOption && (
          ...
          >
            <div>
              <label className="text-sm font-medium block mb-2" style={{ color: '#E0E0E0' }}>
                Quantity
              </label>
            ...
            <div className="rounded-lg p-4" style={{ background: '#0D0D0D' }}>
              <p className="text-sm mb-1" style={{ color: '#E0E0E0' }}>Required Credits: {requiredCredits}</p>
        ...
        <DialogFooter>
          ...
          <Button
            ...
            className="transition-all duration-200 hover:bg-green-600 active:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500"
            style={{ 
